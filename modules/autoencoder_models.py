...
from modules import (
    utils,
    sgd_manager as sgd_m,
    training_constants as tconst 
)
...
class BetaVAE(nn.Module):
    ...
    def loss(self, recon_x, x, mu, logvar, _ids, device):

        # reconstruction losses are summed over all elements and batch
        recon_loss = F.binary_cross_entropy(recon_x, x, reduction='sum')
        kl_diverge = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
        original_loss = (recon_loss + self.beta * kl_diverge) / x.shape[0]  # divide total loss by batch size

        corr_loss, normalized_corr_loss, sgd_loss, iList, tList, cList = sgd_m.compute_SGD_loss(self, x, _ids, device)
        
        combined_loss = original_loss + tconst.CORR_LOSS_COEFF*corr_loss
        
        return combined_loss, original_loss, normalized_corr_loss, sgd_loss
    ...
    def only_corr_loss(self, x, image_ids, device):

        corr_loss, normalized_corr_loss, sgd_loss, r_df = sgd_m.generate_subgroups(self, x, image_ids, device)
        
        return corr_loss, normalized_corr_loss, sgd_loss, r_df


    def only_vae_loss(self, recon_x, x, mu, logvar):
...
class DFCVAE(BetaVAE):
    ...
    def __init__(self, latent_size=100, beta=1, channels_num=3):
        ...
        self.d4 = self._upconv(32, 3)  #TODO
