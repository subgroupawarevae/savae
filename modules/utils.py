...
def restore_latest(net, folder):
    ...
    # So we are taking the list of all .pt files, in sorted order of the file names
    checkpoints = sorted(glob.glob(folder + '/*.pt'), key=os.path.getmtime)
    start_it = 0

    # If there are actually any checkpoints that have been saved
    if len(checkpoints) > 0:

        # Take the one with largest no in its file name
        restore(net, checkpoints[-1])

        try:
            # I think this is the number in the name 
            start_it = int(re.findall(r'\d+', checkpoints[-1])[-1])
        except:
            pass
    
    # Then we return that number. This means that if we order 400 epochs to be done. But 
    # In the file name it is clearly indicated that we have model that has been run for 200
    # epochs, then we load that model in and start from 200 epochs in. Smart.
    return start_it
...
class Logger():
    def __init__(self, file_path):
        ...
        aux.create_folder(os.path.dirname(file_path))
...
# To use this images needs to be a list
